...
class HotkeyManager:
    ...
    def check_startup_status(self):
        ...
        plist_path = launch_agents_path / "com.powerkey.hotkey.plist"
        return plist_path.exists()

    def toggle_startup(self):
        """Toggle the startup status of the application"""
        if sys.platform != 'darwin':
            messagebox.showwarning("Warning", "Startup configuration is only supported on macOS")
            return
    ...
    def update_hotkeys_list(self):
        ...
        for key, action in self.hotkeys.items():
            ...
            hotkey_text = f"Cmd+Shift+Option+{key}"
            ttk.Label(row_frame,
                    text=hotkey_text,
...
2. Using Hotkeys:
   • Press Cmd + Shift + Option + your command key
...
4. Startup Settings:
   • Click the Startup button to toggle auto-start with macOS
...
"""
    ...
    def add_hotkey(self):
        ...
        self.status_var.set(f"Added hotkey: Cmd+Shift+Option+{key}")

    def delete_hotkey(self, key):
        del self.hotkeys[key]
        self.save_hotkeys()
        self.update_hotkeys_list()
        self.status_var.set(f"Deleted hotkey: Cmd+Shift+Option+{key}")
    ...
    def execute_action(self, action):
        try:
            if action.lower() == 'calc':
                ...
                messagebox.showwarning("Warning", "Microsoft Edge is not supported on macOS")
                return
            else:
    ...
    def check_hotkey(self, key):
        # Check for cmd+shift+option combination
        if isinstance(key, keyboard.KeyCode):
            ...
            if all(listener.canonical(k) in modifiers for k in listener._press_cache):
                key_char = key.char.upper()
                if key_char in self.hotkeys:
                    self.execute_action(self.hotkeys[key_char])

    def start_listener(self):
        listener = keyboard.Listener(on_press=self.check_hotkey)
        listener.start()
