...
from AppKit import NSApp, NSApplication, NSMenu, NSMenuItem, NSStatusBar
...
class HotkeyManager:
    def __init__(self):
        ...
        self.themes = {
            'dark': {
                'bg': '#1e1e1e',  # Darker background for dark mode
                'fg': '#ffffff',
                'accent': '#0069c0',  # macOS blue accent
                'button': '#2d2d2d',
                'entry': '#3d3d3d',
                'hover': '#505050',
                'border': '#404040',
                'success': '#28c941',  # macOS green
                'error': '#ff453a',    # macOS red
                'button_text': '#ffffff'
            },
            'light': {
                'bg': '#f5f5f7',  # Light gray background typical in macOS
                'fg': '#000000',
                'accent': '#0069c0',  # macOS blue accent
                'button': '#e0e0e0',
                'entry': '#ffffff',
...
/8AA////////wAA="""
        ...
        tempFile = "icon.icns"  # Use .icns format for macOS
        ...
        self.language_texts = {
            'English': {
                ...
                'warning': 'Warning',
                'startup_warning': 'Startup configuration is not supported on this system',
            ...
            'Spanish': {
                ...
                'warning': 'Advertencia',
                'startup_warning': 'La configuración de inicio no es compatible con este sistema',
            ...
            'French': {
                ...
                'startup_warning': 'La configuration de démarrage n\'est pas prise en charge sur ce système',
            ...
            'German': {
                ...
                'warning': 'Warnung',
                'startup_warning': 'Die Startkonfiguration wird auf diesem System nicht unterstützt',
            ...
            'Swedish': {
                ...
                'warning': 'Varning',
                'startup_warning': 'Startkonfiguration stöds inte på det här systemet',
        ...
        self.setup_macos_application()

        # Setup status bar item
        self.setup_status_bar_item()

    def setup_macos_application(self):
        # Configure application as a proper macOS app
        NSApplication.sharedApplication()
        NSApp.setActivationPolicy_(NSApplication.ActivationPolicyRegular)
        ...
        about_item = NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(
            f"About {self.language_texts[self.settings['language']]['title']}",
            "showAbout:", "")
        ...
        quit_item = NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(
            f"Quit {self.language_texts[self.settings['language']]['title']}",
            "terminate:", "q")
    ...
    def setup_status_bar_item(self):
        # Create a status bar item (menubar icon)
        self.statusbar = NSStatusBar.systemStatusBar()
        self.statusitem = self.statusbar.statusItemWithLength_(-1)  # Variable length

        # Set the title or image for the status item
        self.statusitem.setTitle_("⌘⇧⌥")  # Shows macOS symbols for Cmd+Shift+Option
        ...
        show_item = NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(
            "Show PowerKey", "showApp:", "")
        self.status_menu.addItem_(show_item)

        toggle_item = NSMenuItem.alloc().initWithTitle_action_keyEquivalent_(
            "Enable Hotkeys", "toggleHotkeys:", "")
        ...
        try:
            ...
            if file_path:
                ...
                for key, action in import_data['hotkeys'].items():
                    preview_text.insert(tk.END, f"⌘⇧⌥+{key}: {action}\n")
    ...
    def check_startup_status(self):
        """Check if app is set to run at login via Launch Agents."""
        plist_path = os.path.expanduser(f"~/Library/LaunchAgents/com.powerkey.app.plist")
        return os.path.exists(plist_path)

    def toggle_startup(self):
        """Toggle app launch at login."""
        plist_path = os.path.expanduser(f"~/Library/LaunchAgents/com.powerkey.app.plist")
        ...
        self.startup_var.set(self.language_texts[self.settings['language']]['startup'] +
                            (" ON" if self.startup_enabled else " OFF"))
    ...
    5. Startup Settings:
    • Click the Startup button to toggle auto-start with the system
    ...
    • Examples:
        - notepad (or your system's default text editor)
        - "/Applications/Some App.app"
        - "/usr/bin/Some App"
        - cmd.exe /c echo Hello (if applicable)
        - open "path/to/your/Documents" (Example macOS command)
    ...
    10. Troubleshooting:
        ...
        • For startup issues:
        - Check the system's Task Manager or Activity Monitor
    ...
    2. Usar Atajos:
    • Presiona Cmd + Shift + Option + tu tecla de comando
    ...
    • Haz clic en el botón de Inicio para alternar el inicio automático con el sistema
    ...
    • Ejemplos:
        - notepad (or el editor de texto predeterminado de su sistema)
        - "/Applications/Some App.app"
        - "/usr/bin/Some App"
        - cmd.exe /c echo Hello (si es applicable)
        - open "path/to/your/Documents" (Ejemplo de comando de macOS)
    ...
    10. Solución de Problemas:
        ...
        - Revisa el Administrador de Tareas o el Monitor de Actividad del sistema
    ...
    • Appuyez sur Cmd + Shift + Option + votre touche de commande
    ...
    • Cliquez sur le bouton Démarrage pour activer ou désactiver le démarrage automatique avec le système
    ...
    • Exemples:
        - notepad (or l'éditeur de texte par défaut de votre système)
        - "/Applications/Some App.app"
        - "/usr/bin/Some App"
        - cmd.exe /c echo Hello (si applicable)
        - open "path/to/your/Documents" (Example de commande macOS)
    ...
    10. Dépannage:
        ...
        - Vérifiez le Gestionnaire des Tâches ou le Moniteur d'activité du système
    ...
    • 您可以输入任何有效的系统命令或程序路径
    • 示例:
        - notepad (or 您系统的默认文本编辑器)
        - "/Applications/Some App.app"
        - "/usr/bin/Some App"
        - cmd.exe /c echo Hello (如果适用)
        - open "path/to/your/Documents" (Example macOS command)
    ...
    2. Hotkeys verwenden:
    • Drücken Sie Cmd + Shift + Option + Ihre Befehlstaste
    ...
    • Klicken Sie auf die Schaltfläche Start, um den automatischen Start mit dem System zu aktivieren oder zu deaktivieren
    ...
    • Beispiele:
        - notepad (or der Standardtexteditor Ihres Systems)
        - "/Applications/Some App.app"
        - "/usr/bin/Some App"
        - cmd.exe /c echo Hello (falls zutreffend)
        - open "path/to/your/Documents" (Example macOS command)
    ...
    10. Fehlerbehebung:
        ...
        • Bei Startproblemen:
        - Überprüfen Sie den Task-Manager oder die Aktivitätsanzeige des Systems
    ...
    2. Använd genvägar:
    • Tryck på Cmd + Shift + Option + din kommandotangent
    ...
    • Klicka på Start-knappen för att växla automatisk start med systemet
    ...
    • Exempel:
        - notepad (or systemets standardtextredigerare)
        - "/Applications/Some App.app"
        - "/usr/bin/Some App"
        - cmd.exe /c echo Hello (om tillämpligt)
        - open "path/to/your/Documents" (Example macOS command)
    ...
    10. Felsökning:
        ...
        - Verifiera att programmet inte redan körs
        • För startproblem:
        - Kontrollera systemets Aktivitetskontroll
        - Kontrollera användarkontobehörigheter
    ...
    def add_hotkey(self):
        ...
        self.status_var.set(self.language_texts[self.settings['language']]['added_hotkey'] + f"⌘⇧⌥+{key}")

    def delete_hotkey(self, key):
        del self.hotkeys[key]
        self.save_hotkeys()
        self.update_hotkeys_list()
        self.status_var.set(self.language_texts[self.settings['language']]['deleted_hotkey'] + f"⌘⇧⌥+{key}")

    def save_hotkeys(self):
        config_path = self.get_hotkeys_path()
    ...
    def execute_action(self, action):
        try:
            ...
            elif action.lower() == 'safari':
                # Add Safari as a macOS-specific option
                subprocess.Popen(['open', '-a', 'Safari'])
            elif action.lower() == 'edge':
                # Edge on macOS
                subprocess.Popen(['open', '-a', 'Microsoft Edge'])
            else:
                # For other commands, use open or direct execution
                if os.path.exists(action):
                    # If it's a file path, use 'open'
    ...
    def check_hotkey(self, e):
        ...
        if (self.settings['hotkeys_enabled'] and
            ...
            keyboard.is_pressed('cmd+shift+alt') and
    ...
    def setup_hotkey_listener(self):
        # Use pynput for cross-platform hotkey detection
        self.hotkey_listener = keyboard.GlobalHotKeys({
            # Define the global hotkey combinations
        })
    ...
    def register_hotkeys(self):
        ...
        # Create mapping of key combinations to callbacks
        hotkey_mapping = {}

        # For each defined hotkey in our settings
        for key, action in self.hotkeys.items():
            # Create the key combination string for pynput
            # Format: "<cmd>+<shift>+<alt>+<key>"
            combo = f"<cmd>+<shift>+<alt>+{key.lower()}"

            # Add to our mapping with a lambda that captures the action
            hotkey_mapping[combo] = lambda a=action: self.execute_action(a)

        # Create and start the listener with our mapping
    ...
    def get_settings_path(self):
        """Get macOS-appropriate settings path."""
        return os.path.expanduser('~/Library/Application Support/PowerKey/settings.json')

    def get_hotkeys_path(self):
        """Get macOS-appropriate hotkeys storage path."""
        return os.path.expanduser('~/Library/Application Support/PowerKey/hotkeys.json')

    def ensure_app_directories(self):
        """Ensure application directories exist."""
        app_dir = os.path.expanduser('~/Library/Application Support/PowerKey')
